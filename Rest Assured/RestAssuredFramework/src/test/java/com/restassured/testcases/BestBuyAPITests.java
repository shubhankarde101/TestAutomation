...
public class BestBuyAPITests extends BaseTest {
	...
	/*
	 * Please make sure you have followed these instruction to run the server on
	 * your localhost to execute these test
	 * https://github.com/BestBuy/api-playground
	 * ****************************************** git clone
	 * https://github.com/bestbuy/api-playground/ cd api-playground npm install npm
	 * start ***************************************** Load https://localhost:3030
	 * on your browser
	 * 
	 */

	/*
	 * There should be a test case matching this test name in RUNMANAGER and
	 * TESTDATA sheet If there are multiple data lines in TESTDATA sheet, it will
	 * treated as iteration Mark Yes in RUNMANAGER and TESTDATA to run this test
	 * case
	 * 
	...
	public void getProducts(Hashtable<String, String> data) {
		...
		Response response = given().filter(new RequestLoggingFilter(captor)) // This line is mandatory to log the
				...
				.log().all().get(Constants.BASEURL_BESTBUY
						+ Constants.BESTBUY_GETPRODUCTS_ENDPOINT.replace("{limit}", data.get("limit")));

		/*
		 * Writing the request and response to extent report This statement needs to
		 * repeated in all your tests to log the req and response in extent report.
		 * 
		 * 
		 */
	...
	public void postProductWithoutPOJO(Hashtable<String, String> data) {
		/*
		 * sample request body { "name": "string", "type": "string", "price": 0,
		 * "shipping": 0, "upc": "string", "description": "string", "manufacturer":
		 * "string", "model": "string", "url": "string", "image": "string",
		 * "manufacturer":"string" }
		 */
		...
		mapobj.put("price", Integer.parseInt(RandomUtils.generateRandomNumericString(2))); // Generate a random 2 digit
																							// number on fly
		...
		Response response = given().filter(new RequestLoggingFilter(captor)) // This line is mandatory to log the
																				// request details to extent report
				.header("Content-Type", "application/json").contentType(ContentType.JSON).log().all().body(mapobj) // passing
																													// mapobj
																													// in
																													// request
																													// body
	...
	public void postProductWithPojo(Hashtable<String, String> data) {
		...
		/*
		 * Create a pojo class for the request Create a object and init all class
		 * variables using constructor Pass that in the body of the request
		 */
		...
		PostProductRequest obj = new PostProductRequest(data.get("name"), data.get("type"),
				...
				Integer.parseInt(RandomUtils.generateRandomNumericString(2)), RandomUtils.generateRandomString(3),
				RandomUtils.generateRandomString(10), RandomUtils.generateRandomString(3),
				RandomUtils.generateRandomString(6), RandomUtils.generateRandomString(4),
				RandomUtils.generateRandomString(3));

		Response response = given().filter(new RequestLoggingFilter(captor)) // This line is mandatory to log the
																				// request details to extent report
				.header("Content-Type", "application/json").contentType(ContentType.JSON).log().all().body(obj) // passing
																												// obj
																												// in
																												// request
																												// body
	...
	public void postProductByReadingRequestFromFile(Hashtable<String, String> data) throws IOException  {
		...
		// reading the request body from a json file directly and passing to body as a
		// string
		Response response = given().filter(new RequestLoggingFilter(captor)) // This line is mandatory to log the
																				// request details to extent report
