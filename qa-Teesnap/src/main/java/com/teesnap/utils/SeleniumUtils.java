...
public class SeleniumUtils {
    ...
    public static void waitForPageLoad(WebDriver driver) {
        ...
        while (System.currentTimeMillis() < endTime) {
            ...
            SeleniumUtils.hardWait(interval); // Wait for the next poll
        }
    }

    public static boolean waitForText(WebDriver driver, By locator, String textToAppear, int timeoutSeconds, int pollingMillis) {
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeoutSeconds));
        wait.pollingEvery(Duration.ofMillis(pollingMillis));

        try {
            return wait.until((ExpectedCondition<Boolean>) driverInstance -> {
            ...
            return false; // Return false if timeout occurs or an exception is thrown
        }
    }
