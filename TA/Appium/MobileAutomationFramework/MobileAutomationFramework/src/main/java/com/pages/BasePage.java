...
public class BasePage {
	...
	protected static By btn_hamburger = By
			.xpath("//android.widget.ImageButton[@content-desc='Open navigation drawer']");
	protected static By btn_backinmenubar = By
			.xpath("//android.view.ViewGroup[contains(@resource-id,'toolbar')]/ImageButton");
	...
	protected void longPress(WebElement webelement) {
		ta = new TouchAction(DriverManager.getDriver());
		ta.longPress(longPressOptions().withElement(element(webelement)).withDuration(ofSeconds(4))).release()
				.perform();
	}

	protected void swipe(WebElement source, WebElement target) {
		ta = new TouchAction(DriverManager.getDriver());
		ta.longPress(longPressOptions().withElement(element(source)).withDuration(ofSeconds(3))).moveTo(element(target))
				.release().perform();
	}
	/*
	 * protected void scollToSpecificElement(String text) { //driver.
	 * findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()).scrollIntoView(text(\"WebView\"))"
	 * ); ---->hardcoding //---->Below one is dynamic DriverManager.getDriver().
	...
	public void scrollToSpecificElementandClick(String locatorname, String androidlocatorvalue,
			String ioslocatorvalue) {
		if (DriverManager.getIsAndroid()) {
			if (locatorname.equalsIgnoreCase("xpath")) {
